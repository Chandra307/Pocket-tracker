...
async function sendGetRequest(page) {
    try {
        ...
        const { data: { expenses, pageData } } = await axios.get(`http://13.48.13.12/expense/getexpenses?page=${page}&number=${number}`, { headers: { "Authorization": token } });
...
document.querySelector('form').onsubmit = async (e) => {
    try {
        ...
        if(!putId){
            await axios.post('http://13.48.13.12/expense/addexpense', expenseDetails, { headers: { "Authorization": token } });
            sendGetRequest(1);
            document.querySelector('dialog').close();
        }
        else {
            await axios.put(`http://13.48.13.12/expense/editexpense/${putId}`, expenseDetails, { headers: { "Authorization": token } });
...
window.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        const { data: { files, premium } } = await axios.get('http://13.48.13.12/user/downloads', { headers: { "Authorization": token } });
        if (premium) {
...
document.getElementById('premium').onclick = async (e) => {
    try {
        const response = await axios.get('http://13.48.13.12/premium/purchase', { headers: { "Authorization": token } });
        var options = {
            ...
            "handler": async function (result) {
                try {
                    await axios.post('http://13.48.13.12/premium/updateStatus', {
...
function showLeaderboard() {
    ...
    document.querySelector('#leader').onclick = async (e) => {
        try {
            ...
            const response = await axios.get('http://13.48.13.12/premium/leaderboard');
...
async function deleteExpense(id) {
    try {
        ...
        if (confirm('Delete this expense?')) {
            ...
            await axios.delete(`http://13.48.13.12/expense/delete-expense/${id}`, { headers: { "Authorization": token } });
...
async function editExpense(id) {
    try {
        ...
        const { data: { expense } } = await axios.get(`http://13.48.13.12/expense/getexpense/${id}`, { headers: { "Authorization": token } });
